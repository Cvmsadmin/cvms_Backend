...
public class RegistrationHelper { 
	private static final Class<?> CLASS_NAME = RegistrationHelper.class;
//	private static UPPCLLogger logger = UPPCLLogger.getInstance(UPPCLLogger.MODULE_REGISTRATION,CLASS_NAME.toString());
	
	/**
     * For populated data from CustResponseDTO into CustomerDetailsDTO.
     * 
     * @param formdata
     * @return CustomerDetailsDTOString methodName = request.getRequestURI();
		logger.logMethodStart(methodName);
     */
    public CustomerDetailsModel populateUserDetailsInDTO(CustomerDetailsDTO customrResDTO) {
    	final String methodName = "populateUserDetailsInDTO(CustomerDetailsDTO customrResDTO)";
//		logger.logMethodStart(methodName);
        ...
        custDetailsDTO.setBookNo(customrResDTO.getBookNo());
        String billingAddress = customrResDTO.getBillingAddresss().getAddressLine1() + ","
                ...
                + customrResDTO.getBillingAddresss().getState() + "," + customrResDTO.getBillingAddresss().getPinCode();
        ...
        String currentAddress = customrResDTO.getPremiseAddress().getAddressLine1() + ","
                ...
                + customrResDTO.getPremiseAddress().getState() + "," + customrResDTO.getPremiseAddress().getPinCode();
        ...
        custDetailsDTO.setSubDivision(customrResDTO.getSubDivision());
        String discom = customrResDTO.getDiscom();
        discom = discom.split("-")[0].toString();
        if (null != discom)
        	custDetailsDTO.setDiscomName(discom.trim());
...
//        logger.log(UPPCLLogger.LOGLEVEL_INFO, methodName, "@@@@ 1. Customer Details DTO : : " + custDetailsDTO);
//        logger.logMethodEnd(methodName);
        return custDetailsDTO;
    } 
    
    public static String getConfigFile(String filePath) throws RequestNotFoundException, Exception{
    	final String methodName = "getConfigFile(String filePath)";
//		logger.logMethodStart(methodName);
		Reader reader = null;
...
//			logger.log(UPPCLLogger.LOGLEVEL_INFO, methodName, "@@@@ 1. Exception while reading the File from location: : : " + ex.getMessage());
		} catch (IOException ex) {
//			logger.log(UPPCLLogger.LOGLEVEL_INFO, methodName, "@@@@ 1. Exception while retrieving the file"+ex.getMessage());
		}finally {
			try {
				if(null != resource){
					resource.close();
...
//				logger.log(UPPCLLogger.LOGLEVEL_INFO, methodName, "@@@@ 2.Exception Occured when read file : " + " "+ ex.getMessage());
			}
		}
//		logger.logMethodEnd(methodName);
		return configFile;
