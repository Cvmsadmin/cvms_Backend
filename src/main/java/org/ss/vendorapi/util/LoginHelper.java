...
public class LoginHelper {
	...
	private static final Class<?> CLASS_NAME = LoginHelper.class;
//	private static UPPCLLogger logger = UPPCLLogger.getInstance(UPPCLLogger.MODULE_BILLING,CLASS_NAME.toString());
	...
	public CustomerDetailsDTO populateUserDetailsInDTO(CustomerDetailsDTO customerDetailsDTO,
...
//		logger.logMethodStart(methodName);
		CustomerDetailsDTO customerDetailsDTOObj = new CustomerDetailsDTO();
		...
		customerDetailsDTOObj.setBookNo(customerDetailsDTO.getBookNo());
		String billingAddress = customerDetailsDTO.getBillingAddresss().getAddressLine1() + ","
		...
		+ customerDetailsDTO.getBillingAddresss().getPinCode();
		billingAddress = billingAddress.replace(",,,", ",");
		billingAddress = billingAddress.replace(",,", ",");
		customerDetailsDTOObj.setBillingAddress(billingAddress);

		String currentAddress = customerDetailsDTO.getPremiseAddress().getAddressLine1() + ","
		...
		+ customerDetailsDTO.getPremiseAddress().getPinCode();
		...
		customerDetailsDTOObj.setDivision(customerDetailsDTO.getDivision());
		/*try{
			String [] data = customerResDTO.getDivision().split("\\$");
			if(data.length>1){
				customerDetailsDTO.setDivision(data[0]);
				customerDetailsDTO.setDivCode(data[1]);
			}
		}catch(Exception e){}*/
		
		customerDetailsDTOObj.setSubDivision(customerDetailsDTO.getSubDivision());
//		customerDetailsDTOObj.setGroupName("Consumer");
		customerDetailsDTOObj.setOnlineBillingStatus(customerDetailsDTO.getOnlineBillingStatus());
		...
		customerDetailsDTOObj.setCustomerIndexNumber(customerDetailsDTO.getCustomerIndexNumber());
		...
		BigDecimal amount = new BigDecimal(customerDetailsDTO.getAccountInfo());
		if (null != customerDetailsDTO.getAccountInfo()) {
			customerDetailsDTOObj.setDueBillAmt(amount.setScale(0, BigDecimal.ROUND_HALF_UP).toString());
		} else
...
//			customerDetailsDTOObj.setDueDate(formatter.format(billDetailsResDTO.getBillDueDate()));
...
//			customerDetailsDTOObj.setDueAmount(billDetailsResDTO.getPaymentDue() + "");
		...
		customerDetailsDTOObj.setTypeOfConnection(customerDetailsDTO.getTypeOfConnection());
//		logger.logMethodEnd(methodName);
		return customerDetailsDTOObj;
	}
	
	public CustomerDetailsDTO getConsumerDetails(String kno,String discomName,Environment env) throws RequestNotFoundException{
		final String methodName = "getConsumerDetails";
//		logger.logMethodStart(methodName);
		CustomerDetailsDTO customerDetailsDTO = new CustomerDetailsDTO();
		customerDetailsDTO =ServiceDataUtil.getConsumerDetails(kno,discomName,env);//ConsumerDetails Rest API
	      	//when consumer detail service data available start
		CustomerDetailsDTO custDetailDTO = new CustomerDetailsDTO();
        if(customerDetailsDTO!=null) {
			boolean tdco =  false;
			tdco = ("T").equalsIgnoreCase(customerDetailsDTO.getMothersName());
			ArrayList<String> secondryAcc = new ArrayList<String>();
			if ((Constants.TRUE).equalsIgnoreCase(env.getProperty("relatedConsumerFlag"))) {
				GetRelatedConsumerHelper getRelatedConsumerHelper = new GetRelatedConsumerHelper();
				GetRelatedConsumerDTO getRelatedConsumerDTO = new GetRelatedConsumerDTO();
				getRelatedConsumerDTO = getRelatedConsumerHelper.populateGetRelatedConsumerDTO(customerDetailsDTO);
//						List<RelatedConsumerResponseDTO> relatedConsumerResponseDTO = consumerSOAAccessor.getRelatedConsumers(getRelatedConsumerDTO);
//						if (relatedConsumerResponseDTO != null) {
//							if (logger.isDebugLoggingEnabled())
//								logger.log(UPPCLLogger.LOGLEVEL_DEBUG, METHOD_NAME, new StringBuilder("Account No : ").append(relatedConsumerResponseDTO.get(0).getAccountNo()).toString());
							// secondryAcc.add(customerResDTO.getKnumber());
//							for (RelatedConsumerResponseDTO relatedConsumers : relatedConsumerResponseDTO) {
//								secondryAcc.add(relatedConsumers.getAccountNo());
						secondryAcc.add(customerDetailsDTO.getKno());//temp
//							}
			}
			else {
				secondryAcc.add(customerDetailsDTO.getKno());
			}
			List<BillDetailsResDTO> billDetailsResDTOList = ServiceDataUtil.getBillingDetails(kno,customerDetailsDTO.getDiscom(),env);
			if(billDetailsResDTOList == null) {
				throw new RequestNotFoundException("Bill Details Not Found");
			}
			else {
				LoginHelper loginHelper = new LoginHelper();
				if (billDetailsResDTOList != null && billDetailsResDTOList.size() > 0) {
					
					custDetailDTO = loginHelper.populateUserDetailsInDTO(customerDetailsDTO, secondryAcc,billDetailsResDTOList.get(0));
					BillDisplayDTO billDisplayDTO = ServiceDataUtil.getBillDisplayDetails(kno,customerDetailsDTO.getDiscom(),env);
...
//							}
				...
				custDetailDTO.setDateOfBirth(customerDetailsDTO.getDateOfBirth());
				if ("true".equalsIgnoreCase(custDetailDTO.getConfirmEmail())) {
...
////						if(knoDiscom !=null)
////							updateStatus = customerCredentialsRepository.updateLastLogin(loginDate, knoDiscom);
////						else
////							updateStatus = customerCredentialsRepository.updateLastLogin(loginDate, customerDetailsDTO.getKno());
				}
//						else {
//						//get URL for confirm Email and put it into Hash map.
////						loginHelper.sendMailWithDisplayErrMsg(pageUrl, custDetailDTO,knoDiscom,custDetailDTO.getDiscomName());//shikha
//						}
			}
		}
		else {
